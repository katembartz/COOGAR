...
def D(f, dx, dy, dz, Nx, Ny, Nz, device): 
    ...
    c[start_x:stop_x, start_y:stop_y, start_z:stop_z] = torch.zeros((Nx-abs(dx)-1, Ny-abs(dy)-1, Nz-abs(dz)-1), dtype=torch.float64, device=device)
    v = v - c
...
def D_T(f):
    ...
    out = torch.zeros(Nx, Ny, Nz, Nb, dtype=f.dtype, device=f.device)
